<?php
Class REQuiz_model extends CI_Model
{
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('reexam',$userdata);
 ...
 function update_quiz($exam_id){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->update('reexam',$userdata);
	  
	  $this->db->where('exam_id',$exam_id);
	  $query=$this->db->get('reexam',$userdata);
...
$noq_subject=array();
	...
	//print_r($userdata);exit;
	 ...
	 $this->db->insert('reexam_result',$userdata);
 ...
 function open_result($exam_id,$uid){
	 $result_open='open';
		$query=$this->db->query("select * from reexam_result  where result_status='$result_open' "); 
...
return $result['rid'];		
 ...
 function quiz_result1($rid){
	...
	$query=$this->db->query("select reexam_result.* from reexam_result join reexam on reexam_result.exam_id=reexam.exam_id where reexam_result.rid='$rid' "); 
	return $query->row_array(); 
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from reexam_result as result join reexam on result.exam_id=reexam.exam_id where result.rid='$rid' "); 
	return $query->row_array(); 
...
function saved_answers($rid){
 ...
 function assign_score($rid,$qno,$score){
	 $qp_score=$score;
	 $query=$this->db->query("select * from reexam_result join reexam on result.exam_id=reexam.exam_id where result.rid='$rid' "); 
	$quiz=$query->row_array(); 
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('reexam_result',$userdata);
	 ...
	 foreach($_POST['answer'] as $ak => $answer){	 
		 // multiple choice single answer	 
			...
			//	return	print_r($userdata);exit;
					$this->db->insert('reexam_answers',$userdata);
